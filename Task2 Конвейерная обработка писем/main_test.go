...
func TestTwoPipelinesWithHasSpam(t *testing.T) {
	// Тест проверяет, что два пайплайна с HasSpam работают корректно
	// и не превышают глобальный лимит параллельных запросов
	...
	// Первый пайплайн (более длинный)
	go func() {
		defer wg.Done()
		RunPipeline(
			cmd(newCatStrings(inputData1, 0)),
			cmd(SelectUsers),
			cmd(SelectMessages),
			cmd(CheckSpam),
			cmd(func(in, out chan interface{}) {
				for item := range in {
					mu.Lock()
					results1 = append(results1, fmt.Sprintf("%v", item))
					mu.Unlock()
				}
			}),
		)
	...
	// Второй пайплайн (стартует с задержкой)
	go func() {
		time.Sleep(100 * time.Millisecond) // Стартуем позже
		defer wg.Done()
		RunPipeline(
			cmd(newCatStrings(inputData2, 0)),
			cmd(SelectUsers),
			cmd(SelectMessages),
			cmd(CheckSpam),
			cmd(func(in, out chan interface{}) {
				for item := range in {
					mu.Lock()
					results2 = append(results2, fmt.Sprintf("%v", item))
					mu.Unlock()
				}
			}),
		)
	...
	t.Run("NoDeadlock", func(t *testing.T) {
		// Главное - не было deadlock или паники
		assert.True(t, true, "пайплайны завершились без deadlock")
	})

	t.Run("ReasonableTime", func(t *testing.T) {
		// Время должно быть разумным (не слишком большим из-за блокировок)
		maxExpected := 5 * time.Second
		assert.Less(t, timeEnd, maxExpected,
			"пайплайны должны работать параллельно, а не последовательно")
	})

	t.Run("HasSpamErrorsWithinLimit", func(t *testing.T) {
		// Ошибок "too many requests" должно быть мало или вообще не быть
		// если семафоры работают правильно
		assert.Less(t, stat.ErrorHasSpam, uint32(5),
			"не должно быть много ошибок 'too many requests'")
	})

	t.Run("AllDataProcessed", func(t *testing.T) {
		// Проверяем, что оба пайплайна обработали данные
		assert.Greater(t, len(results1), 0, "первый пайплайн должен получить результаты")
		assert.Greater(t, len(results2), 0, "второй пайплайн должен получить результаты")

		totalResults := len(results1) + len(results2)
		assert.Greater(t, totalResults, 10, "должно быть обработано значительное количество сообщений")
	})

	// Логируем статистику для отладки
	t.Logf("Время выполнения: %v", timeEnd)
	t.Logf("Ошибок HasSpam: %d", stat.ErrorHasSpam)
	t.Logf("Вызовов HasSpam: %d", stat.RunHasSpam)
	t.Logf("Результатов в пайплайне 1: %d", len(results1))
	t.Logf("Результатов в пайплайне 2: %d", len(results2))
}
